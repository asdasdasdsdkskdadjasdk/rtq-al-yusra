...
export default function AdminUangMasuk({ auth, history, siswa, pending, filters }) {
    ...
    const { 
        ...
        post: postPay, processing: procPay, reset: resetPay 
    } = useForm({
        jumlah_bayar: '', 
        keterangan: '', 
        tanggal_bayar: new Date().toISOString().split('T')[0]
    ...
    const { 
        ...
        reset: resetEdit, errors: errorsEdit 
    } = useForm({
        jumlah_bayar: '',
        tanggal_bayar: '',
        keterangan: ''
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="flex space-x-1 rounded-xl bg-gray-200 p-1 w-fit">
                        ...
                        <button onClick={() => setActiveTab('siswa')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === 'siswa' ? 'bg-white shadow text-blue-700' : 'text-gray-600'}`}>
                            Daftar Siswa
                        </button>
                        <button onClick={() => setActiveTab('history')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === 'history' ? 'bg-white shadow text-blue-700' : 'text-gray-600'}`}>
                            History
                        </button>
                    </div>
                    <div className="w-full sm:w-64">
                        <input type="text" placeholder="Cari Nama Siswa..." className="w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-lg shadow-sm text-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                </div>
                            ...
                            <tbody>
                                {pending.length > 0 ? pending.map((item) => (
                                    <tr key={item.id} className="border-b">
                                        <td className="p-3 font-bold">{item.uang_masuk?.user?.name}</td>
                                        <td className="p-3 text-green-600">Rp {parseInt(item.jumlah_bayar).toLocaleString()}</td>
                                        <td className="p-3">{item.bukti_bayar ? <a href={`/storage/${item.bukti_bayar}`} target="_blank" className="text-blue-500 underline">Lihat</a> : '-'}</td>
                                        <td className="p-3 flex gap-2">
                                            <Link href={route('admin.uang_masuk.approve', item.id)} method="post" as="button" className="bg-green-500 text-white px-2 py-1 rounded text-xs">Terima</Link>
                                            <Link href={route('admin.uang_masuk.reject', item.id)} method="post" as="button" className="bg-red-500 text-white px-2 py-1 rounded text-xs" onSuccess={() => confirm('Tolak?')}>Tolak</Link>
                                        </td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan="4" className="p-6 text-center text-gray-400">Tidak ada pembayaran menunggu konfirmasi.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                            ...
                            <tbody>
                                {siswa.length > 0 ? siswa.map((item) => (
                                    <tr key={item.id} className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => setDetailUser(item)}>
                                        ...
                                        <td className="p-3 font-medium">{item.is_beasiswa ? <span className="text-purple-600 font-bold">Beasiswa (Rp 0)</span> : `Rp ${item.total_tagihan.toLocaleString()}`}</td>
                                    ...
                                    <tr><td colSpan="6" className="p-6 text-center text-gray-500 italic">Data siswa tidak ditemukan.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                ...
                {activeTab === 'history' && (
                    <div className="bg-white p-6 rounded-xl shadow-sm overflow-x-auto">
                        <div className="flex justify-between mb-4">
                            <h3 className="font-bold">History Pembayaran</h3>
                            <button onClick={() => setShowManualModal(true)} className="bg-blue-600 text-white px-4 py-2 rounded text-xs font-bold">+ Input Manual</button>
                        </div>
                        <table className="w-full text-sm text-left">
                            <thead className="bg-gray-50 text-gray-700 uppercase"><tr><th className="p-3">Tgl</th><th className="p-3">Nama</th><th className="p-3">Jml</th><th className="p-3">Ket</th><th className="p-3">Oleh</th><th className="p-3 text-center">Aksi</th></tr></thead>
                            <tbody>
                                {history.length > 0 ? history.map((item) => (
                                    <tr key={item.id} className="border-b hover:bg-gray-50">
                                        <td className="p-3 text-gray-500">{new Date(item.tanggal_bayar).toLocaleDateString()}</td>
                                        <td className="p-3 font-medium">{item.uang_masuk?.user?.name}</td>
                                        <td className="p-3 text-green-600 font-bold">+ {parseInt(item.jumlah_bayar).toLocaleString()}</td>
                                        <td className="p-3 text-gray-500 italic">{item.keterangan}</td>
                                        <td className="p-3 text-xs">{item.pencatat?.name ?? 'Sistem'}</td>
                                        <td className="p-3 text-center">
                                            <div className="flex justify-center gap-2">
                                                <button onClick={() => openEditRiwayat(item)} className="text-blue-600 hover:text-blue-800 text-xs font-bold underline">Edit</button>
                                                {/* FIX: Gunakan handleDeleteRiwayat, bukan deleteHistory */}
                                                <button onClick={() => handleDeleteRiwayat(item.id)} className="text-red-600 hover:text-red-800 text-xs font-bold underline">Hapus</button>
                                            </div>
                                        </td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan="6" className="p-6 text-center text-gray-500 italic">Belum ada riwayat pembayaran yang cocok.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                ...
                <div className="p-6">
                    ...
                    <form onSubmit={handleManualTransaction} className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            <input type="number" className="w-full border rounded p-2" value={manualData.jumlah_bayar} onChange={e => setManualData('jumlah_bayar', e.target.value)} placeholder="Nominal (Rp)" required />
                        </div>
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <button disabled={procManual} className="px-4 py-2 bg-blue-600 text-white rounded">Simpan</button>
                        </div>
                    </form>
                </div>
            </Modal>
                        ...
                        {!detailUser.is_beasiswa && (
                            <form onSubmit={handleBayarDetail} className="bg-gray-50 p-4 rounded-lg mb-6 border border-gray-200">
                                ...
                                <div className="grid grid-cols-3 gap-2 mb-2">
                                    ...
                                    <input type="number" placeholder="Nominal" className="col-span-2 w-full border rounded p-1.5 text-xs" value={payData.jumlah_bayar} onChange={e => setPayData('jumlah_bayar', e.target.value)} />
                                </div>
                                <div className="flex gap-2">
                                    ...
                                    <button disabled={procPay} className="bg-blue-600 text-white px-3 py-1 rounded text-xs font-bold">+ Simpan</button>
                                </div>
                            </form>
                        )}
                                    ...
                                    {detailUser.riwayat.map(r => (
                                        <tr key={r.id} className="border-b">
                                            ...
                                            <td className="p-2 text-center flex justify-center gap-2">
                                                <button onClick={() => openEditRiwayat(r)} className="text-blue-600 hover:text-blue-800 text-xs font-bold underline">Edit</button>
                                                {/* FIX: Gunakan handleDeleteRiwayat, bukan deleteHistory */}
                ...
                <div className="p-6">
                    ...
                    <form onSubmit={handleUpdateRiwayat} className="space-y-4">
                        <div>
                            ...
                            <input type="date" className="w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" value={editData.tanggal_bayar} onChange={(e) => setEditData('tanggal_bayar', e.target.value)} required />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Nominal Bayar (Rp)</label>
                            <input type="number" className="w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" value={editData.jumlah_bayar} onChange={(e) => setEditData('jumlah_bayar', e.target.value)} required />
                            {errorsEdit.jumlah_bayar && <div className="text-red-500 text-xs mt-1">{errorsEdit.jumlah_bayar}</div>}
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Keterangan</label>
                            <input type="text" className="w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" value={editData.keterangan} onChange={(e) => setEditData('keterangan', e.target.value)} placeholder="Contoh: Transfer via BCA..." />
                        </div>
                        <div className="flex justify-end gap-2 mt-6">
                            ...
                            <button type="submit" disabled={procEdit} className="px-4 py-2 bg-blue-600 text-white rounded-md font-medium hover:bg-blue-700 transition disabled:opacity-50">{procEdit ? 'Menyimpan...' : 'Simpan Perubahan'}</button>
                        </div>
                    </form>
                </div>
            </Modal>

...
const NavItemWithDropdown = ({ item, auth, isScrolled, initialTextColor }) => {
    ...
    return (
        ...
        >
            <button
                className={`font-medium transition hover:text-alyusra-orange inline-flex items-center ${item.href ? '' : 'cursor-default'} ${isScrolled ? 'text-gray-700' : initialTextColor
                    }`}
                onClick={(e) => { if (!item.href) e.preventDefault(); }}
            >
                {item.text}
                {visibleSubmenus.length > 0 && (
                    <svg className={`w-4 h-4 ml-1 transition-transform ${isOpen ? 'rotate-180' : ''} ${isScrolled ? 'text-gray-700' : initialTextColor
...
export default function Navbar({ auth, heroTheme }) {
    ...
    const navLinks = [
        ...
        { href: route('kelulusan.index'), text: 'Cek Kelulusan', guestOnly: true }, // FIX: guestOnly
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    ...
                    <nav className="hidden md:flex items-center space-x-8">
                        {navLinks.map((link) => (
                            link.submenu ? (
                                ...
                                // Logic Complex: Show if (Not AuthOnly OR Auth+User) AND (Not GuestOnly OR Guest+NoUser)
                                ((!link.authOnly || (link.authOnly && auth?.user)) &&
                                    (!link.guestOnly || (link.guestOnly && !auth?.user))) && (
                                    <Link
                                        key={link.text}
                                        href={link.href}
                    ...
                    <div className="hidden md:flex items-center space-x-4">
                        {auth && auth.user ? (
                            <>
                                <span className={`font-semibold transition ${isScrolled ? 'text-gray-700' : initialTextColor
                                    }`}>{auth.user.name}</span>
                    ...
                    <div className="md:hidden">
                        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className={`transition hover:text-alyusra-orange ${isScrolled ? 'text-gray-700' : initialTextColor
                            }`}>
                            {isMobileMenuOpen ? (
                        ...
                        ((!link.authOnly || (link.authOnly && auth?.user)) &&
                            ...
                            link.submenu ? (
                                ...
                                <div key={link.text}>
                                    ...
                                    {link.submenu.map((sub) => (
                                        <Link
                                            key={sub.text}
                                            href={sub.href}
                                            className={`block pl-6 px-3 py-2 rounded-md text-base font-medium hover:text-alyusra-orange transition ${isScrolled ? 'text-gray-700 hover:bg-gray-100' : 'text-white hover:bg-white/10'
                                                }`}
                                ...
                                <Link
                                    key={link.text}
                                    href={link.href || '#'}
                                    className={`block px-3 py-2 rounded-md text-base font-medium hover:text-alyusra-orange transition ${link.customColor // Terapkan warna custom juga di mobile
                                        ? link.customColor
